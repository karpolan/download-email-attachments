...
module.exports = function findEmails (args, callback) {
  // console.log('findEmails() args:', args);
  ...
  var maybeErrorCallback = once(function () {
    ...
    if (!errorFree) {
      ...
      callback({
        ...
        lastSyncId: state.newLastSyncId,
        uidvalidity: state.uidvalidity,
        handledIds: extraHandledIds,
        errors: state.errors
      })
    } else {
      // eslint-disable-next-line
  ...
  function handleEmail (result, resultCallback) {
    ...
    fetcher.on('message', function (msg, seqno) {
      ...
      parser.on('attachment', function (attachment, mail) {
        ...
        saveAttachmentStream(args, attachment, mail, onAttStreamFinished, function (errC) {
          collectErrors(errC, result)
        })
      })
